        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200">
            ...
            {formsWithErrors.map((f) => (
              ...
              >
                <td className="px-4 py-2 text-sm font-medium text-gray-900">{f.form}</td>
                <td className="px-4 py-2 text-sm text-center text-gray-600">{f.total}</td>
                ...
                <td className="px-4 py-2 text-sm text-center text-gray-400">{f.skipped}</td>
                ...
                <td className="px-4 py-2 text-center">
                  {f.failed > 0 ? (
                    ...
                    <span className="text-xs text-gray-400">Sin reglas aplicables</span>
                  )}
                </td>
              </tr>
            ))}
            ...
            {showCorrectForms &&
              formsWithoutErrors.map((f) => (
                <tr key={f.form} className={activeForm === f.form ? 'bg-blue-50' : 'bg-white'}>
                  ...
                  <td className="px-4 py-2 text-sm text-center text-gray-600">{f.total}</td>
                  <td className="px-4 py-2 text-sm text-center text-green-600">{f.passed}</td>
                  <td className="px-4 py-2 text-sm text-center text-gray-400">0</td>
                  <td className="px-4 py-2 text-sm text-center text-gray-400">{f.skipped}</td>
                  <td className="px-4 py-2 text-sm text-center font-medium">
                    <span className={cumplimientoColor(f.cumplimiento)}>
                      {f.cumplimiento !== null ? `${f.cumplimiento}%` : '—'}
