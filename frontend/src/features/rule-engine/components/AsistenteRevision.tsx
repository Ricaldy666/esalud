...
const SYNC_STATUS_MAP: Record<string, { label: string; color: string }> = {
  pendiente: { label: 'Pendiente', color: 'text-slate-500' },
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            {row.profesional && <p className="text-sm text-slate-500">{row.profesional}</p>}
            <span className="text-xs text-slate-400 font-mono">Fila n° {row.row_number}</span>
          </div>
                ...
                {idx < STEPS.length - 1 && (
                  <div
                    className={`flex-1 h-0.5 mx-2 rounded ${
                      currentStep > s.position ? 'bg-emerald-400' : 'bg-slate-200'
                    }`}
                  />
                )}
              </div>
        ...
        <>
          <div className="flex gap-2">
            {[1, 2].map((s) => (
              <div
                key={s}
                className={`flex-1 h-1.5 rounded-full transition-colors ${s <= step ? 'bg-indigo-500' : 'bg-slate-200'}`}
              />
            ))}
          </div>
          ...
          {step === 1 && (
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-slate-900">Comportamiento funcional</h3>
              </div>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {info && <p className="text-xs text-slate-500 mt-0.5">{info.tooltip}</p>}
                        </div>
                      </div>
                    </label>
                  )
          ...
          {step === 2 && (
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-slate-900">Fundamentación (opcional)</h3>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm resize-none h-20 outline-none focus:border-indigo-300 focus:ring-1 focus:ring-indigo-300"
              ...
              <div className="bg-slate-50 rounded-lg border border-slate-200 p-4 space-y-2">
                ...
                <div className="text-sm space-y-1">
                  <p>
                    <span className="font-medium text-slate-700">Fila {row.row_number}</span> —{' '}
                    {row.concepto ?? '—'}
                  </p>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  className="flex-1 rounded-lg border border-slate-200 px-3 py-2 text-xs outline-none focus:border-indigo-300 focus:ring-1 focus:ring-indigo-300"
                />
              </div>
      ...
      <div className="border-t border-slate-200 pt-3">
        ...
        {showTecnico && (
          <div className="mt-2 space-y-3">
            <div className="bg-slate-50 rounded-lg border border-slate-200 p-3 space-y-1.5 text-xs text-slate-600">
              <p>
                <span className="font-medium text-slate-700">Patrón:</span> {getPatternLabel(row)}
              </p>
              <p>
                <span className="font-medium text-slate-700">Alcance:</span> {row.scope ?? 'global'}
              </p>
              <p>
              ...
              {row.parent_criterion_id && (
                <p>
                  <span className="font-medium text-slate-700">Hereda de:</span> criterio #
                  {row.parent_criterion_id}
                </p>
              )}
            </div>
              ...
              <div className="bg-slate-50 rounded-lg border border-slate-200 p-3 space-y-2">
                ...
                <div className="space-y-1.5">
                  {workflowData.history.map((h) => (
                    <div key={h.id} className="text-[11px] text-slate-500 flex items-start gap-2">
                      <div className="shrink-0 mt-0.5">
                        <div className="w-1.5 h-1.5 rounded-full bg-indigo-300" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className="font-medium text-slate-700">{h.from_state.name}</span>
                        <span className="text-slate-400 mx-1">→</span>
                        <span className="font-medium text-slate-700">{h.to_state.name}</span>
                        {h.transitioned_by && (
