...
export const ErrorGroupCard: React.FC<Props> = ({ group }) => {
  ...
  const borderClass = severityBorder[group.severidad] ?? 'border-l-slate-300'
  const bgClass = severityBg[group.severidad] ?? ''

  return (
    <div
      className={`bg-white rounded-lg border border-slate-200 border-l-[5px] ${borderClass} ${bgClass} overflow-hidden`}
    >
      <button
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-sm font-semibold text-slate-900">{group.form}</span>
              <span className="text-xs text-slate-400">/</span>
              <span className="text-sm text-slate-600">Sección {group.section}</span>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-xs">
              {group.variables.map((v) => (
                <span key={v} className="font-medium text-slate-800">
                  • {v}
                </span>
              ))}
            </div>
